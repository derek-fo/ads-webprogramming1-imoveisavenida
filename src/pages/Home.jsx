  ...
  return (
    <div className="bg-[#F7F6F2] min-h-screen flex flex-col font-sans antialiased">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8 flex-1 w-full">
        ...
        {/* Categorias Deslizantes Mobile-Friendly */}
        <div className="flex gap-6 sm:gap-8 overflow-x-auto pb-3 mb-6 scrollbar-none border-b border-[#BCC5CC]/30 snap-x">
          {categorias.map((cat) => (
            <button
              key={cat.value}
              onClick={() => setCategoria(cat.value)}
              className={`pb-2 border-b-2 whitespace-nowrap transition-all text-xs sm:text-sm font-bold snap-start -mb-[2px] ${
                categoria === cat.value
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3 bg-white rounded-2xl border border-[#BCC5CC]/40 max-w-md mx-auto px-4">
            ...
            <h3 className="text-base font-bold text-[#063154]">Nenhum imóvel encontrado</h3>
